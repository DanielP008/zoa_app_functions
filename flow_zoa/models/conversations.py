...
class ZoaConversation:
    ...
    def send(self, request_json):
        ...
        # 1. Extracción de Identificadores (unificado para evitar errores de nombre)
        company_id = (
        ...
        if msg_type == "text":
            ...
            # Construimos el payload específico para texto con el nombre de campo corregido
            final_payload = {
            ...
            # Si no hay conversation_id, usamos el destinatario directo 'to'
            if not final_payload["conversation_id"]:
                final_payload["to"] = request_json.get("to") or request_json.get("phone")
                # Algunas APIs fallan si envías la llave conversation_id como null
        ...
        # --- CASO TEMPLATE ---
            ...
            # Resolución del ID del template
    ...
    def assign(self, request_json):
        """
        Asigna una conversación a un usuario según la doc de ZOA.
        Si ya existe, se actualiza automáticamente.
        """
        ...
        # 1. Búsqueda de ID si no viene en el request
        ...
        resolved_user_id = u_data[0].get("id") if isinstance(u_data, list) and u_data else u_data.get("id") if isinstance(u_data, dict) else None

        if not resolved_user_id:
            return {"error": "No se pudo extraer el UUID del usuario"}, 400

        # 3. Petición POST al endpoint de la Doc
        try:
            url_assign = f"{self.api_base}/waba/conversations/{conv_id}/assign"
            
            # Ajuste según la doc: "Request body containing the user_id to assign"
            # Cambiamos de 'user_ids' (lista) a 'user_id' (string)
            payload = {
                "user_id": str(resolved_user_id)
            }
            
            print(f"DEBUG: Intentando asignar a {manager_name} ({resolved_user_id})")
            response = requests.post(url_assign, headers=self.headers, json=payload)
            
            if response.status_code == 422:
                print(f"DEBUG 422 - Respuesta ZOA: {response.text}")
    ...
    def status(self, request_json):
        """
        Actualiza el sales_status de una conversación.
        Sigue el patrón de la documentación de assign proporcionada.
        """
        conv_id = request_json.get("conversation_id") or request_json.get("id")
        customer_phone = request_json.get("phone") or request_json.get("customer_phone")
        company_id = request_json.get("company_id")
        ...
        # 1. Localizar el ID de la conversación (Fallback por teléfono)
        if not conv_id and customer_phone:
            url_list = f"{self.api_base}/waba/conversations?phone_number_id={company_id}"
            res = requests.get(url_list, headers=self.headers)
            if res.status_code == 200:
        ...
        if not conv_id:
            return {"error": "No se encontró el ID de la conversación"}, 404

        # 2. Petición al endpoint específico de sales-status
        try:
            # Siguiendo la lógica de tu doc de /assign, el endpoint de status es:
            url_status = f"{self.api_base}/waba/conversations/{conv_id}/sales-status"
            
            # Payload siguiendo el esquema de FastAPI (llave única)
            payload = {
                "sales_status": new_status
            }
            
            print(f"DEBUG: Enviando POST a {url_status} con payload {payload}")
            # Nota: Muchas funciones de cambio de estado en FastAPI/ZOA son POST
            response = requests.post(url_status, headers=self.headers, json=payload)
            
            # Si da 405 (Método no permitido), intentamos PATCH
            if response.status_code == 405:
                print("DEBUG: POST no permitido, intentando PATCH...")

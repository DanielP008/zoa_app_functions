...
class ZoaConversation2:
    ...
    def send(self, request_json):
        ...
        if msg_type == "text":
            ...
            conv_id = self._ensure_conv_id(request_json, company_id)
            final_payload = {
            ...
            if not template_id:
                name = request_json.get("template_name")
                if name:
        ...
        try:
            response = requests.post(f"{self.api_base.rstrip('/')}{endpoint}", headers=self.headers, json=final_payload, timeout=15)
    ...
    def assign_status(self, request_json):
        ...
        return {
            "status": "success",
            "conversation_id": conv_id,
            "results": {"assign": a_res, "status": s_res}
        }, 200
